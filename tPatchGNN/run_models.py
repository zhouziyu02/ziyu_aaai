...
parser.add_argument('--use_hyperParam_optim', type=int, default = 0, help = 'True: HyperParameters optimization using optuna, False: no optimization')

parser.add_argument(
    '--no_hyperParam_optim',
    action='store_false',
    dest='use_hyperParam_optim',
    help='禁用超参优化'
)
...
if __name__ == '__main__':
    if args.use_hyperParam_optim:
        tuner = Tuner(ranSeed=42, n_jobs=2)  # 或者n_jobs>1并行搜索
        tuner.tune(args)
    else:
        train_main(args)  # 直接训练
